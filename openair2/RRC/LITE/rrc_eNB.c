...
#endif
//#define XER_PRINT
int rrc_eNB_process_HandoverRequest(MessageDef *msg_p, const char *msg_name, instance_t instance);
#ifdef PHY_EMUL
...
//-----------------------------------------------------------------------------
{
...
#endif
    LOG_I(RRC, "UE HANDOVER INFO %c\n", ue_context_pP->ue_context.handover_info->ho_prepare);
    if (ue_context_pP->ue_context.handover_info->ho_prepare != 0xF0)
    {
        ...
        printf("[eNB_RRC] MCC 1:[%d],2:[%d],3:[%d]\n",
               *measResults2->measResultNeighCells->choice.measResultListEUTRA.list.array[0]->cgi_Info->cellGlobalId.plmn_Identity.mcc->list.array[0],
               *measResults2->measResultNeighCells->choice.measResultListEUTRA.list.array[0]->cgi_Info->cellGlobalId.plmn_Identity.mcc->list.array[1],
               *measResults2->measResultNeighCells->choice.measResultListEUTRA.list.array[0]->cgi_Info->cellGlobalId.plmn_Identity.mcc->list.array[2]);
        fflush(stdout);
        printf("MNC LIST 1:[%d],2:[%d]\n",
               *measResults2->measResultNeighCells->choice.measResultListEUTRA.list.array[0]->cgi_Info->cellGlobalId.plmn_Identity.mnc.list.array[0],
               *measResults2->measResultNeighCells->choice.measResultListEUTRA.list.array[0]->cgi_Info->cellGlobalId.plmn_Identity.mnc.list.array[1]);
        fflush(stdout);
        ...
        {
            t = true;
            rrc_eNB_generate_HandoverPreparationInformation_125t(ctxt_pP,
                    ue_context_pP,
                    measResults2->measResultNeighCells->choice.
                    measResultListEUTRA.list.array[0]);
            LOG_I(RRC, "DO THIS\n");
        }
    }
    else
...
void
rrc_eNB_generate_HandoverPreparationInformation_125t(
    const protocol_ctxt_t *const ctxt_pP,
    rrc_eNB_ue_context_t *const ue_context_pP,
    MeasResultEUTRA_t *measResultEUTRA_t
)
{
    PhysCellId_t targetPhyId = measResultEUTRA_t->physCellId;
    //新建目的上下文，用于传递给核心网
    ...
    //对天线数量和频率进行配置
    handoverInfo->as_config.antennaInfoCommon.antennaPortsCount = 12;    //Not used 0,but 1 is also disallow
    handoverInfo->as_config.sourceDl_CarrierFreq = 36090;   //Verify!

    //填充handoverinfo中的as_config
    ...
    memcpy((void *)&ue_context_pP->ue_context.handover_info->as_config.sourceSystemInformationBlockType1,
           (void *)&eNB_rrc_inst[ctxt_pP->module_id].carrier[0].SIB1, sizeof(SystemInformationBlockType1_t));

    memcpy((void *)&ue_context_pP->ue_context.handover_info->as_config.sourceSystemInformationBlockType2,
           (void *)&eNB_rrc_inst[ctxt_pP->module_id].carrier[0] .SIB23, sizeof(SystemInformationBlockType2_t));

    //填充as_context，目前感觉某些值不是太准确
    ...
    ue_context_pP->ue_context.handover_info->as_context.reestablishmentInfo->sourcePhysCellId =
        eNB_rrc_inst[ctxt_pP->module_id].carrier[0] .physCellId;
    ...
    //ue_context_pP->ue_context.handover_info->ho_prepare = 0xF0;    //0xF0;
    ue_context_pP->ue_context.handover_info->ho_complete = 0;

    //这里对目标基站的id进行一个判断
    if (mod_id_target != 0xFF)
    {
        //根据目标基站的mod_id尝试获得ue_context
        ue_context_target_p = rrc_eNB_get_ue_context(&eNB_rrc_inst[mod_id_target], ue_context_pP->ue_context.rnti);

        if (ue_context_target_p == NULL)   //说明此时目标基站还没有关于该UE的上下文
        {
            //进行目标上下文的分配(该上下文是用来传递给目标基站的!)
            ...
            LOG_N(RRC,
                  ...
                  ctxt_pP->module_id,
                  mod_id_target);
            //将ue_context_pP的handover_info内容复制到target上下文里面
            ...
            rrc_eNB_S1AP_send_handover_require(ctxt_pP, ue_context_pP, measResultEUTRA_t);
        }
        else     //说明ue_context此时已经处于目标基站
        {
            LOG_E(RRC, "\nError in obtaining free UE id in target eNB %ld for handover 125\n", targetPhyId);
        }
    }
    else     //说明是错误的目标基站
    {
        LOG_E(RRC, "\nError in obtaining Module ID of target eNB for handover 125\n");
    }
}
//-----------------------------------------------------------------------------
void
        ...
        if (ue_context_p->ue_context.handover_info != NULL)
        {
            ...
            if (ue_context_p->ue_context.handover_info->ho_complete == 0xF1)
            {
                LOG_I(RRC,
                      ...
                      ctxt_pP->module_id,
                      ue_context_p->ue_context.handover_info->modid_t);
                rrc_eNB_process_handoverPreparationInformation(ctxt_pP, ue_context_p);
...
//-----------------------------------------------------------------------------
{
    ...
    *quantityConfig->quantityConfigEUTRA->filterCoefficientRSRQ = FilterCoefficient_fc4;
    ...
    //ZJR
    mobilityInfo->targetPhysCellId = 1;
    ...
    LOG_I(RRC, "[eNB %d] Frame %d: handover preparation: targetPhysCellId: %ld mod_id: %d ue: %x \n",
          ctxt_pP->module_id,
          ctxt_pP->frame,
          mobilityInfo->targetPhysCellId,
          ctxt_pP->module_id,
    ...
    *mobilityInfo->additionalSpectrumEmission = 1;  //Check this value!
    ...
    ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.srb_ToAddModList = SRB_configList2;
    //memcpy((void *)rrc_inst->handover_info[ue_mod_idP]->as_config.sourceRadioResourceConfig.drb_ToAddModList,(void *)DRB_list,sizeof(DRB_ToAddModList_t));
    ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.drb_ToAddModList = DRB_configList2;
    ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.drb_ToReleaseList = NULL;
    ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.mac_MainConfig =
        CALLOC(1, sizeof(*ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.mac_MainConfig));
    memcpy((void *)ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.mac_MainConfig,
    ...
    memcpy((void *)ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.physicalConfigDedicated,
           (void *)ue_context_pP->ue_context.physicalConfigDedicated, sizeof(PhysicalConfigDedicated_t));
    ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.sps_Config = NULL;
    ue_context_pP->ue_context.handover_info->size = sizeof(ue_context_pP->ue_context.handover_info);
    ...
    MSC_LOG_TX_MESSAGE(
        ...
        buffer,
        size,
        MSC_AS_TIME_FMT" TomDing rrcConnectionReconfiguration_handover UE %x MUI %d size %u",
        MSC_AS_TIME_ARGS(ctxt_pP),
        ue_context_pP->ue_context.rnti,
        rrc_eNB_mui,
        size);
    ...
    if (sourceModId != 0xFF)
    {
        uint8_t sourceModId = ctxt_pP->module_id;
...
*/
uint16_t
rrc_eNB_generate_RRCConnectionReconfiguration_handover_125t(
    const protocol_ctxt_t *const ctxt_pP,
    rrc_eNB_ue_context_t *const ue_context_pP,
    uint8_t *buffer,
    PhysCellId_t targetPhyId,
...
//-----------------------------------------------------------------------------
{
    ...
    *mobilityInfo->additionalSpectrumEmission = 1;  //Check this value!
    ...
    mobilityInfo->newUE_Identity.size = 2;
    mobilityInfo->newUE_Identity.bits_unused = 0;
    mobilityInfo->newUE_Identity.buf=malloc(2*sizeof(uint8_t));
    memcpy(mobilityInfo->newUE_Identity.buf, &ue_context_pP->ue_context.rnti, 2 * sizeof(uint8_t));
    ...
    // store the srb and drb list for ho management, mainly in case of failure
    printf("[HO REQUEST] flag1\n");
    fflush(stdout);
    ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.srb_ToAddModList=malloc(sizeof(SRB_ToAddModList_t));
    ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.drb_ToAddModList=malloc(sizeof(DRB_ToAddModList_t));
    ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.mac_MainConfig=malloc(sizeof(MAC_MainConfig_t));
    ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.physicalConfigDedicated=malloc(sizeof(PhysicalConfigDedicated_t));
    memcpy(ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.srb_ToAddModList,
           (void *)SRB_configList2,
           sizeof(SRB_ToAddModList_t));
    printf("[HO REQUEST] flag2\n");
    fflush(stdout);
    memcpy((void *)ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.drb_ToAddModList,
           (void *)DRB_configList2,
           sizeof(DRB_ToAddModList_t));
    printf("[HO REQUEST] flag3\n");
    fflush(stdout);
    ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.drb_ToReleaseList = NULL;
    memcpy((void *)ue_context_pP->ue_context.handover_info->as_config.sourceRadioResourceConfig.mac_MainConfig,
           (void *)mac_MainConfig,
...
#   endif
    ...
    if (ul_dcch_msg->message.present == UL_DCCH_MessageType_PR_c1)
    {
        ...
        case UL_DCCH_MessageType__c1_PR_rrcConnectionReconfigurationComplete:
            printf("TomDing Received Reconfiguration Msg\n");
            fflush(stdout);
#ifdef RRC_MSG_PRINT
...
#endif
            if ( ctxt_pP->module_id != NULL)
            {
                printf("TomDing module_id=%d\n", ctxt_pP->module_id);
            }
            if (ctxt_pP->frame != NULL)
            {
                printf("TomDing frame=%d\n", ctxt_pP->frame);
            }
            if (ctxt_pP->rnti != NULL)
            {
                printf("TomDing rnti=%d\n", ctxt_pP->rnti);
            }
...
//-----------------------------------------------------------------------------
{
    ...
    while (1)
    {
        ...
        case S1AP_HANDOVER_REQUEST:
            //name modify
            rrc_eNB_process_HandoverRequest(msg_p, msg_name_p, instance);
            break;
#   endif
...
int rrc_eNB_process_HandoverRequest(MessageDef *msg_p, const char *msg_name, instance_t instance)
{
    ...
    protocol_ctxt_t ctxt;
    /*
    1、生成随机的random_value和rnti
    2、通过rnti获取新的protocol_ctxt
    3、通过protocol_ctxt和random_value得到新的rrc_eNB_ue_context_s
    4、随便分配Preamble_index用于透传封装
    */
    rnti_t rnti;
    uint64_t random_value = 0;
    //根据targetCellId获取到当前目标基站的module_id
    uint8_t mod_id_target = get_adjacent_cell_mod_id(S1AP_HANDOVER_REQUEST(msg_p).targetCellId);
    if (mod_id_target != 0xFF)
    {
        //只有当前module_id存在才可以进行切换,module_id为小区标识
        ue_context_target_p = rrc_eNB_get_ue_context(&eNB_rrc_inst[ctxt.module_id], rnti);
        if (ue_context_target_p == NULL)
        {
            ...
            memcpy(((uint8_t *) & random_value) + 3, rv, 5);
            ...
            memcpy(&rnti, rv, 2);
            int preambleIndex = 62; //随便分配的一个非竞争随机接入index
            PROTOCOL_CTXT_SET_BY_MODULE_ID(&ctxt, mod_id_target, ENB_FLAG_YES, rnti, 0, 0, mod_id_target);
            printf("[HO REQUEST] [eNB %d] HandoverRequest rnti:%d\n", ctxt.module_id, ctxt.rnti);
            printf("[HO REQUEST] random_value=%x\n", random_value);
            fflush(stdout);
            ...
            ue_context_target_p->ue_context.handover_info->modid_t = mod_id_target;
            ue_context_target_p->ue_context.handover_info->modid_s = ctxt.module_id;
            ue_context_target_p->ue_context.handover_info->ueid_t  = ue_context_target_p->ue_context.rnti;
            ...
            gtpv1u_enb_create_tunnel_resp_t create_tunnel_resp;
            {
                ...
                ue_context_target_p->ue_context.nb_of_e_rabs = nb_e_rabs_tosetup;
                create_tunnel_req.rnti = ue_context_target_p->ue_context.rnti; // warning put zero above
                create_tunnel_req.num_tunnels = nb_e_rabs_tosetup;
                ...
                ue_context_target_p->ue_context.setup_e_rabs += nb_e_rabs_tosetup;
            }
            //封装RRCConnectionReconfiguration
            ue_context_target_p->ue_context.handover_info = CALLOC(1, sizeof(*(ue_context_target_p->ue_context.handover_info)));
        ...
        {
            LOG_E(RRC, "\nError in obtaining free UE id in target eNB %ld for handover 125\n", S1AP_HANDOVER_REQUEST(msg_p).targetCellId);
        }
    }
    else
    {
