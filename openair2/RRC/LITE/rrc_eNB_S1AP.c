    ...
    uint16_t container_size)
{
    ...
    handoverCommand.criticalExtensions.choice.c1.choice.handoverCommand_r8.handoverCommandMessage.buf = container_buf;
    handoverCommand.criticalExtensions.choice.c1.choice.handoverCommand_r8.handoverCommandMessage.size = container_size;
    uint8_t *buf = malloc16(8192);
    asn_enc_rval_t ret = uper_encode_to_buffer(&asn_DEF_HandoverCommand, &handoverCommand, buf, 8192);
    printf("[HO REQUEST] ret.encoded:%d\n", ret.encoded);
    fflush(stdout);
...
//封装HandoverPreparationInformation为buffer传递给S1AP层，解析测量报告发送给S1AP层
void rrc_eNB_S1AP_send_handover_require(const protocol_ctxt_t *const ctxt_pP,
                                        rrc_eNB_ue_context_t *const ue_context_pP, MeasResultEUTRA_t *measResultEUTRA_t)
{
    //封装RRC_Container,并发送给S1AP层
    MessageDef *msg_p;
    msg_p = itti_alloc_new_message(TASK_RRC_ENB, S1AP_HANDOVER_REQUIRE);
    //buf is to store the source to target transparent container
    uint8_t *buf;
    buf = malloc16(8192);

    //S1ap_SourceeNB_ToTargeteNB_TransparentContainer_t *S1ap_SourceeNB_ToTargeteNB_TransparentContainer;

    HandoverPreparationInformation_r8_IEs_t *HandoverPreparationInformation_r8_IEs;

    UE_CapabilityRAT_Container_t ue_CapabilityRAT_Container;

    HandoverPreparationInformation_r8_IEs = CALLOC(1, sizeof(HandoverPreparationInformation_r8_IEs_t));
    ...
    memset((void *)&HandoverPreparationInformation_r8_IEs->ue_RadioAccessCapabilityInfo, 0, sizeof(UE_CapabilityRAT_ContainerList_t));
    ...
    OCTET_STRING_fromBuf(&ue_CapabilityRAT_Container.ueCapabilityRAT_Container,
                         (const char *)eNB_rrc_inst[ctxt_pP->module_id].UECapability,
                         eNB_rrc_inst[ctxt_pP->module_id].UECapability_size);
    //package ContainerList

    HandoverPreparationInformation_r8_IEs->ue_RadioAccessCapabilityInfo.list.count = 0;
    ASN_SEQUENCE_ADD(
        &HandoverPreparationInformation_r8_IEs->ue_RadioAccessCapabilityInfo.list,
    ...
    asn_dec_rval_t  dec_rval = uper_decode(NULL,
                 ...
                 eNB_rrc_inst[ctxt_pP->module_id].UECapability,
                 eNB_rrc_inst[ctxt_pP->module_id].UECapability_size, 0, 0);
    printf("consumed:%d\n",dec_rval.consumed);
    fflush(stdout);
    */
    //package AS_Context and AS_Config

    asn_enc_rval_t ret = uper_encode_to_buffer(&asn_DEF_HandoverPreparationInformation_r8_IEs, HandoverPreparationInformation_r8_IEs, buf, 8192);

    AssertFatal (ret.encoded > 0, "ASN1 message encoding failed (%s, %jd)!\n",
                 ret.failed_type->name, ret.encoded);

    /*
    HandoverPreparationInformation_r8_IEs_t *test=NULL;
    asn_dec_rval_t dec_rval1 = uper_decode(NULL,
    ...
    */
    ...
    asn_enc_rval_t ret2 = uper_encode_to_buffer(&asn_DEF_S1ap_SourceeNB_ToTargeteNB_TransparentContainer,S1ap_SourceeNB_ToTargeteNB_TransparentContainer, buf2, 8192);
    printf("TomDing flag 8\n");
    fflush(stdout);
    AssertFatal (ret2.encoded > 0, "ASN1 message encoding failed (%s, %jd)!\n",
           ret2.failed_type->name,ret2.encoded);
    */


    //  if (ret2.encoded == -1) abort();
    //e_rab_params
    ...
    PLMN_Identity_t *pLMNidentity = &measResultEUTRA_t->cgi_Info->cellGlobalId.plmn_Identity;
    target_MNC_Digit_Length = pLMNidentity->mnc.list.count;
    /*
    printf("[HO REQUIRE] MCC:1[%d],2[%d],3[%d]\n", *pLMNidentity->mcc->list.array[0],
           *pLMNidentity->mcc->list.array[1], *pLMNidentity->mcc->list.array[2]);
    fflush(stdout);
    printf("[HO REQUIRE] MNC:1[%d],2[%d]\n", *pLMNidentity->mnc.list.array[0],
           *pLMNidentity->mnc.list.array[1]);
    */
    if (pLMNidentity->mcc->list.count == 2)
    {
        targetMCC = *pLMNidentity->mcc->list.array[0] * 10 + *pLMNidentity->mcc->list.array[1];
    }
    else
    {
        targetMCC = *pLMNidentity->mcc->list.array[0] * 100 + *pLMNidentity->mcc->list.array[1] * 10 + *pLMNidentity->mcc->list.array[2];
    }
    if (pLMNidentity->mnc.list.count == 2)
    {
        targetMNC = *pLMNidentity->mnc.list.array[0] * 10 + *pLMNidentity->mnc.list.array[1];
    }
    else
    {
        targetMNC = *pLMNidentity->mnc.list.array[0] * 100 + *pLMNidentity->mnc.list.array[1] * 10 + *pLMNidentity->mnc.list.array[2];
    }
    if (measResultEUTRA_t->cgi_Info->trackingAreaCode.size == 2)
    {
        targetTAC = (measResultEUTRA_t->cgi_Info->trackingAreaCode.buf[0] << 8) + measResultEUTRA_t->cgi_Info->trackingAreaCode.buf[1];
    }
    printf("[HO REQUIRE] physCellID:%d,targetMCC:%d,targetMNC:%d,targetTAC:%d;\n",
           measResultEUTRA_t->physCellId, targetMCC, targetMNC, targetTAC);
    fflush(stdout);
    //获取eNB_ID
    targeteNBID = BIT_STRING_to_uint32( &measResultEUTRA_t->cgi_Info->cellGlobalId.cellIdentity);
    printf("[HO REQUIRE] targeteNBID:%d\n", targeteNBID);
    fflush(stdout);
    ...
    {
        S1AP_HANDOVER_REQUIRE(msg_p).e_rab_param[i] = ue_context_pP->ue_context.e_rab[i].param;
    }

    S1AP_HANDOVER_REQUIRE(msg_p).rrc_container_buf = buf;
    S1AP_HANDOVER_REQUIRE(msg_p).rrc_container_len = (ret.encoded + 7) / 8;

    S1AP_HANDOVER_REQUIRE(msg_p).ue_initial_id = ue_context_pP->ue_context.ue_initial_id;
    S1AP_HANDOVER_REQUIRE(msg_p).eNB_ue_s1ap_id = ue_context_pP->ue_context.eNB_ue_s1ap_id;
    S1AP_HANDOVER_REQUIRE(msg_p).ue_id_rnti = ue_context_pP->ue_id_rnti;
    S1AP_HANDOVER_REQUIRE(msg_p).local_uid = ue_context_pP->local_uid;
    S1AP_HANDOVER_REQUIRE(msg_p).setup_e_rabs = ue_context_pP->ue_context.setup_e_rabs;
    S1AP_HANDOVER_REQUIRE(msg_p).nb_of_e_rabs = ue_context_pP->ue_context.nb_of_e_rabs;
    S1AP_HANDOVER_REQUIRE(msg_p).sourceCellId = ue_context_pP->ue_context.handover_info->as_context.reestablishmentInfo->sourcePhysCellId;
    S1AP_HANDOVER_REQUIRE(msg_p).modid_t = ue_context_pP->ue_context.handover_info->modid_t;
    S1AP_HANDOVER_REQUIRE(msg_p).modid_s = ue_context_pP->ue_context.handover_info->modid_s;
    S1AP_HANDOVER_REQUIRE(msg_p).targetCellId = measResultEUTRA_t->physCellId;
    S1AP_HANDOVER_REQUIRE(msg_p).targetMCC = targetMCC;
    S1AP_HANDOVER_REQUIRE(msg_p).targetMNC = targetMNC;
    S1AP_HANDOVER_REQUIRE(msg_p).target_MNC_Digit_Length = target_MNC_Digit_Length;
    S1AP_HANDOVER_REQUIRE(msg_p).targeteNBID = targeteNBID;
    S1AP_HANDOVER_REQUIRE(msg_p).targetTAC = targetTAC;
...
//------------------------------------------------------------------------------
{
    ...
    //TomDing Modify;Store the UE Radio Access Capability

    UE_CapabilityRAT_ContainerList_t  list = ueCapabilityInformation->criticalExtensions.choice.c1.choice.ueCapabilityInformation_r8.ue_CapabilityRAT_ContainerList;
    UE_CapabilityRAT_Container_t *container = list.list.array[0];


    eNB_rrc_inst[ctxt_pP->module_id].UECapability = container->ueCapabilityRAT_Container.buf;
    eNB_rrc_inst[ctxt_pP->module_id].UECapability_size = container->ueCapabilityRAT_Container.size;

    printf("TomDing container->ueCapabilityRAT_Container.size:%d\n", container->ueCapabilityRAT_Container.size);
    fflush(stdout);
...
int rrc_eNB_process_S1AP_INITIAL_CONTEXT_SETUP_REQ(MessageDef *msg_p, const char *msg_name, instance_t instance)
{
    ...
    else
    {
...

        {
            ...
            {
                printf("TomDing rrc_eNB_generate_UECapabilityEnquiry flag3 \n");
                fflush(stdout);
