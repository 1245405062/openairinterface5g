...
//module_id应该是eNB_index,index是eNB的第index个小区
...
uint8_t do_MIB(uint8_t Mod_id, LTE_DL_FRAME_PARMS *frame_parms, uint32_t frame, uint8_t *buffer)
{
    ...
    printf("TomDing encode mib to buffer\n");
    fflush(stdout);
#if defined(ENABLE_ITTI)
...
//------------------------------------------------------------------------------
{
    ...
    printf("TomDing  doRRCConnectionReconfiguration\n");
    memset(&dl_dcch_msg, 0, sizeof(DL_DCCH_Message_t));
    ...
    {
        printf("MobilityInfo is not NULL\n");
...
uint8_t do_MeasurementReport(const uint8_t Mod_id, uint8_t const *buffer, const long measid, const long phy_id, const long rsrp_s, const long rsrq_s, const long rsrp_t, const long rsrq_t)
{
    printf("magicwo_tag1:measid%d,do_M", measid);
    printf("magicwo_tag1:do_M");
    fflush(stdout);
    ...
    printf("magicwo_tag4:do_M");
    fflush(stdout);
    //MCC=208,MNC=93
    MCC_MNC_Digit_t dummy1 = 2;
    ASN_SEQUENCE_ADD(&measresult_cgi2->cellGlobalId.plmn_Identity.mcc->list, &dummy1);
    MCC_MNC_Digit_t dummy2 = 0;
    ASN_SEQUENCE_ADD(&measresult_cgi2->cellGlobalId.plmn_Identity.mcc->list, &dummy2);
    MCC_MNC_Digit_t dummy3 = 8;
    ASN_SEQUENCE_ADD(&measresult_cgi2->cellGlobalId.plmn_Identity.mcc->list, &dummy3);
    printf("[ASN1_MSG] MCC 1:[%d],2:[%d],3:[%d]\n",
           *measresult_cgi2->cellGlobalId.plmn_Identity.mcc->list.array[0],
           *measresult_cgi2->cellGlobalId.plmn_Identity.mcc->list.array[1],
           *measresult_cgi2->cellGlobalId.plmn_Identity.mcc->list.array[2]);
    measresult_cgi2->cellGlobalId.plmn_Identity.mnc.list.size = 0;
    measresult_cgi2->cellGlobalId.plmn_Identity.mnc.list.count = 0;
    MCC_MNC_Digit_t dummy4 = 9;
    ASN_SEQUENCE_ADD(&measresult_cgi2->cellGlobalId.plmn_Identity.mnc.list, &dummy4);
    MCC_MNC_Digit_t dummy5 = 3;
    ASN_SEQUENCE_ADD(&measresult_cgi2->cellGlobalId.plmn_Identity.mnc.list, &dummy5);
    printf("[ASN1_MSG] MCN 1:[%d],2:[%d]\n",
           *measresult_cgi2->cellGlobalId.plmn_Identity.mnc.list.array[0],
           *measresult_cgi2->cellGlobalId.plmn_Identity.mnc.list.array[1]);
    fflush(stdout);
    /*
    dummy=2;
    ...
    */
    ...
    measresult_cgi2->cellGlobalId.cellIdentity.buf[2] = 0xe0;
    ...
    */
    ...
    measresult_cgi2->trackingAreaCode.buf[1] = 0x01;
...
OAI_UECapability_t *fill_ue_capability(char *UE_EUTRA_Capability_xer_fname)
{
    ...
    if (!UE_EUTRA_Capability_xer_fname)
    {
        ...
        if(usim_test == 1)
        {
            ...
            printf("TomDing flag 2\n");
        }
        printf("TomDing flag 3\n");
        fflush(stdout);
