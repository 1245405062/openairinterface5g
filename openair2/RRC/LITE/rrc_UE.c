...
//-----------------------------------------------------------------------------
{
    ...
    if (measConfig->reportConfigToAddModList != NULL)
    {
        ...
        for (i = 0; i < measConfig->reportConfigToAddModList->list.count; i++)
        {
            ...
            {
                LOG_I(RRC, "Adding Report Configuration %ld %p \n", ind - 1, measConfig->reportConfigToAddModList->list.array[i]);
    ...
    if (measConfig->measIdToAddModList != NULL)
    {
        for (i = 0; i < measConfig->measIdToAddModList->list.count; i++)
        {
            ...
            {
                LOG_I(RRC, "Modifying Measurement ID %ld\n", ind - 1);
            ...
            {
                LOG_I(RRC, "Adding Measurement ID %ld %p\n", ind - 1, measConfig->measIdToAddModList->list.array[i]);
...
//-----------------------------------------------------------------------------
{
    ...
    printf("%d\n", sizeof(ue_CapabilityRAT_Container));
    fflush(stdout);
    ...
    if (UECapabilityEnquiry->criticalExtensions.present == UECapabilityEnquiry__criticalExtensions_PR_c1)
    {
        if (UECapabilityEnquiry->criticalExtensions.choice.c1.present == UECapabilityEnquiry__criticalExtensions__c1_PR_ueCapabilityEnquiry_r8)
        {
            ...
            ul_dcch_msg.message.choice.c1.choice.ueCapabilityInformation.criticalExtensions.choice.c1.choice.ueCapabilityInformation_r8.ue_CapabilityRAT_ContainerList.list.count
                = 0;
            printf("TomDing Enquiry:%d\n", UECapabilityEnquiry->criticalExtensions.choice.c1.choice.ueCapabilityEnquiry_r8.ue_CapabilityRequest.list.count);
            fflush(stdout);
...
//-----------------------------------------------------------------------------
{
    ...
    if (rrcConnectionReconfiguration->criticalExtensions.present == RRCConnectionReconfiguration__criticalExtensions_PR_c1)
    {
                ...
                RRCConnectionReconfiguration__criticalExtensions__c1_PR_rrcConnectionReconfiguration_r8)
        {
            ...
            printf("TomDing Flag-1\n");
            fflush(stdout);
            printf("magicwo dao le  measConfig qian mian");
            if (rrcConnectionReconfiguration_r8->measConfig)
            {
                LOG_I(RRC, "Measurement Configuration is present\n");
                printf("magicwo jinqule measConfig ");
                rrc_ue_process_measConfig(ctxt_pP,
                                          eNB_index,
                                          rrcConnectionReconfiguration_r8->measConfig);

            }
            printf("TomDing Flag0\n");
            fflush(stdout);
        ...
        if (UE_rrc_inst[ctxt_pP->module_id].Info[eNB_indexP].State >= RRC_CONNECTED)
        {
            ...
            case DL_DCCH_MessageType__c1_PR_rrcConnectionReconfiguration:
                ...
                if (dl_dcch_msg->message.choice.c1.choice.rrcConnectionReconfiguration.criticalExtensions.choice.c1.choice.rrcConnectionReconfiguration_r8.mobilityControlInfo
                        != NULL)
                {
                    printf("TomDing UE_rrc_inst[ctxt_pP->module_id].HandoverInfoUe.targetCellId=%d\n", UE_rrc_inst[ctxt_pP->module_id].HandoverInfoUe.targetCellId);
                    fflush(stdout);
                    /* 36.331, 5.3.5.4 Reception of an RRCConnectionReconfiguration including the mobilityControlInfo by the UE (handover)*/
                            ...
                            != dl_dcch_msg->message.choice.c1.choice.rrcConnectionReconfiguration.criticalExtensions.choice.c1.choice.rrcConnectionReconfiguration_r8.mobilityControlInfo->targetPhysCellId)
                    {
                        ...
                        return;
                             ...
                             == 0xFF)
                    {
                        printf("TomDing  the target_eNB_index=%d\n", target_eNB_index );
                        LOG_I(RRC,
                ...
                printf("TomDing UE process RRCConnectionReconfiguration\n");
                fflush(stdout);
                if (dl_dcch_msg->message.choice.c1.choice.rrcConnectionReconfiguration.criticalExtensions.choice.c1.choice.rrcConnectionReconfiguration_r8.mobilityControlInfo
                        == NULL)
                {
                ...
                {
                    printf("TomDing target_eNB_index get target_eNB%d\n", target_eNB_index);
...
static int decode_SIB1( const protocol_ctxt_t *const ctxt_pP, const uint8_t eNB_index, const uint8_t rsrq, const uint8_t rsrp )
{
    ...
    printf("TomDing decode_SIB1mcc:%d,mnc:%d,tac:%d,plmn_ind:%d\n", mcc, mnc, tac, plmn_ind);
    fflush(stdout);
...
static void rrc_ue_generate_MeasurementReport(protocol_ctxt_t *const ctxt_pP, uint8_t eNB_index )
{
    ...
    nElem = 98;
    nElem1 = 35;
    target_eNB_offset = 3 /*UE_rrc_inst[ctxt_pP->module_id].Info[0].handoverTarget*/; // eNB_offset of target eNB: used to obtain the mod_id of target eNB
    ...
    for (i = 0; i < MAX_MEAS_ID; i++)
    {
        if (UE_rrc_inst[ctxt_pP->module_id].measReportList[0][i] != NULL)
        {
            ...
            rsrp_t = binary_search_float(RSRP_meas_mapping, nElem, -130); //RSRP of target cell
            rsrq_t = binary_search_float(RSRQ_meas_mapping, nElem1, -12); //RSRQ of target cell
            ...
            //  if (measFlag == 1) {
            cellId = 0 /*get_adjacent_cell_id(ctxt_pP->module_id, eNB_index)*/; //PhycellId of serving cell
            //cellId=two_tier_hexagonal_cellIds[eNB_index];
            ...
            printf("TomDing targetCellId generate Measurement Report :%d\n", targetCellId );
            if (pframe != ctxt_pP->frame)
            {
                pframe = ctxt_pP->frame;
                LOG_I(RRC, "[UE %d] Frame %d: doing MeasReport: servingCell(%ld) targetCell(%ld) rsrp_s(%ld) rsrq_s(%ld) rsrp_t(%ld) rsrq_t(%ld) \n",
                      ctxt_pP->module_id,
                      ctxt_pP->frame,
                      cellId,
                      targetCellId,
...
void ue_measurement_report_triggering(protocol_ctxt_t *const ctxt_pP, const uint8_t eNB_index )
{
    ...
    //printf("magicwo UE_moduleId:%d -- eNB_index:%d",ctxt_pP->module_id,eNB_index);
    //printf("magicwo:eNB_Flag%d",ctxt_pP->enb_flag);
                    ...
                    if(UE_rrc_inst[ctxt_pP->module_id].MeasObj[i][measObjId - 1]->measObject.present == MeasObjectToAddMod__measObject_PR_measObjectEUTRA)
                    {
                                 ...
                                 ReportConfigEUTRA__triggerType_PR_event))
                        {
                            ...
                            case ReportConfigEUTRA__triggerType__event__eventId_PR_eventA1:
                                LOG_I(RRC, "[UE %d] Frame %d : A1 event: check if serving becomes better than threshold\n",
                                      ctxt_pP->module_id, ctxt_pP->frame);
                                break;

                            case ReportConfigEUTRA__triggerType__event__eventId_PR_eventA2:
                                LOG_I(RRC, "[UE %d] Frame %d : A2 event, check if serving becomes worse than a threshold\n",
                                      ctxt_pP->module_id, ctxt_pP->frame);
                                break;

                            case ReportConfigEUTRA__triggerType__event__eventId_PR_eventA3:
                                LOG_I(RRC, "[UE %d] Frame %d : A3 event: check if a neighboring cell becomes offset better than serving to trigger a measurement event \n",
                                      ctxt_pP->module_id, ctxt_pP->frame);
                                LOG_I(RRC, "[magicwo UE]%d,T304:%d,measFlag:%d\n", UE_rrc_inst[ctxt_pP->module_id].Info[0].State, UE_rrc_inst[ctxt_pP->module_id].Info[0].T304_active, UE_rrc_inst[ctxt_pP->module_id].HandoverInfoUe.measFlag);
                                //magicwo_i++;

                                if (magicwo_i == 20)
                                {
                                    UE_rrc_inst->Info[0].handoverTarget = 1;//eNB_index added by magicwo
                                    UE_rrc_inst->HandoverInfoUe.targetCellId = 1; //PhyCellID added by magicwo
...
# if ENABLE_RAL
        ...
        case HANDOVER_TRIGGER:
            ...
            LOG_C(RRC, "[UE %d] Received Handover Trigger MSG \n", ue_mod_id);
            HandoverTrigger *trigger = &HANDOVER_TRIGGER(msg_p);
            printf("flag :%d\n", trigger->flag);
            fflush(stdout);
            magicwo_i = 20;
            break;
