...
/*
  处理Handover Request信令
  根据E-RAB的QoS属性进行接入控制判断，判断成功则可用接收切换
  根据QoS分配预留资源
  解析透传信息，将小区信息发送到RRC层进行目的到源的透传封装
*/
static
int s1ap_eNB_handle_handover_request(uint32_t               assoc_id,
                                     uint32_t               stream,
                                     struct s1ap_message_s *s1ap_message_p)
{
    printf("[HO REQUEST] Receive Handover Request\n");
    fflush(stdout);

    uint8_t receive_handover;
    s1ap_eNB_mme_data_t   *mme_desc_p    = NULL;
    s1ap_eNB_ue_context_t *ue_desc_p     = NULL;
    struct S1ap_E_RABInformationList *source_eRAB_list;
    MessageDef       *message_p    = NULL;
    S1ap_HandoverRequestIEs_t *s1ap_HandoverRequest_p;
    message_p        = itti_alloc_new_message(TASK_S1AP, S1AP_HANDOVER_REQUEST);

    DevAssert(s1ap_message_p != NULL);

    //提取出handoverRequest并进行验证
    s1ap_HandoverRequest_p = &s1ap_message_p->msg.s1ap_HandoverRequestIEs;
    if ((mme_desc_p = s1ap_eNB_get_MME(NULL, assoc_id, 0)) == NULL)
    {
    ...
    //解析HandoverRequest
    S1ap_Source_ToTarget_TransparentContainer_t *source_ToTarget_TransparentContainer = &s1ap_HandoverRequest_p->source_ToTarget_TransparentContainer;
    S1ap_SourceeNB_ToTargeteNB_TransparentContainer_t *S1ap_SourceeNB_ToTargeteNB_TransparentContainer_t;
    asn_dec_rval_t  ret = uper_decode(NULL,
                                      &asn_DEF_S1ap_SourceeNB_ToTargeteNB_TransparentContainer,
                                      (void **)&S1ap_SourceeNB_ToTargeteNB_TransparentContainer_t,
                                      source_ToTarget_TransparentContainer->buf,
                                      source_ToTarget_TransparentContainer->size, 0, 0);
    printf("[HO REQUEST] decode conntainer:%d,consumed:%d\n", ret.code, ret.consumed);
    fflush(stdout);
    //进行QoS判断和Handover Resource Allocation TODO
    /*
    1、如果包含Handover Restriction List IE,包含限制服务的小区
    2、存储UE Security Capabilities用于安全配置 as 33.401
    3、如果包含Trace Activation IE,CSG_ID等进行相关配置
    4、存储UE History List Information用于之后的切换
    5、处理E-RAB,按照E-RAB Setup的流程进行处理
    */
    receive_handover = 1;
    ...
    {
        //TODO
        printf("[HO REQUEST] QoS Reject Access\n");
        fflush(stdout);
    }

    //ue_desc_p->eNB_ue_s1ap_id = 11;
    ...
    S1AP_HANDOVER_REQUEST(message_p).security_capabilities.integrity_algorithms =
        BIT_STRING_to_uint16(&s1ap_HandoverRequest_p->ueSecurityCapabilities.integrityProtectionAlgorithms);

    memcpy(&S1AP_HANDOVER_REQUEST(message_p).security_key,
           s1ap_HandoverRequest_p->securityContext.nextHopParameter.buf, s1ap_HandoverRequest_p->securityContext.nextHopParameter.size);
