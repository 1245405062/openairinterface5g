...
void *s1ap_eNB_task(void *arg)
{
    ...
    while (1)
    {
        ...
        case S1AP_HANDOVER_REQUEST_ACK:
        {
            ...
            fflush(stdout);
            s1ap_send_handover_request_ack(ITTI_MESSAGE_GET_INSTANCE(received_msg), &S1AP_HANDOVER_REQUEST_ACK(received_msg));
        }
        //write by coco
        ...
        {
            printf("by coco:in s1ap_path_switch_request task\n");
            s1ap_path_switch_request(ITTI_MESSAGE_GET_INSTANCE(received_msg), &S1AP_PATH_SWITCH_REQUEST(received_msg));
        }
        break;
